...
> 💬 **REVIEW-OPMERKINGEN, LEES DIT EERST** (toegevoegd door Claude bij een plan-review op 2026-07-07)
>
...
>
> Elke opmerking bevat waar mogelijk geverifieerd bewijs (`bestand:regel`). De verificatie is gedaan tegen de code op branch `release/nijmegen-06-2026`.
>
> **Voorgestelde extra taken** die nog volledig ontbreken staan gebundeld onderaan, net vóór "8. Validatiechecklist".
...
> 💬 **REVIEW-OPMERKING, LOW.** Een throw in `parseJwt` propageert via de getUser-catch (`user.js:65-70`) als request-error (500), niet als anonieme fallback. De validatiechecklist (§8) verwacht "pendingJwt als Bearer -> 401". Vang de `pending`-claim daarom af in `parseAuthHeader` (`user.js:85-112`) en behandel als geen-auth (anoniem), i.p.v. te throwen in `parseJwt`. Dan klopt de 401/anoniem-verwachting.
...
> 💬 **REVIEW-OPMERKING, HIGH (stap 2).** De controllercode gebruikt `if (!uniqueCode || uniqueCode.isUsed) ...`, maar `isUsed` bestaat NIET op het model (`apps/auth-server/model/unique~code.js` heeft alleen `code`/`userId`/`clientId`, geen veld/getter/virtual). De bestaande TokenStrategy leest `uniqueCode.isUsed` ook al (`apps/auth-server/auth.js:104,107`) en krijgt altijd `undefined`, dus de "reeds gebruikt"-reject is dode code. Gevolg: unique codes (stemcodes) zijn vandaag de facto herbruikbaar, en dit endpoint erft dat. Beslis expliciet: OF echte single-use implementeren (migratie `usedAt`/`isUsed` + zetten bij login, ook in de bestaande flow), OF documenteren dat codes herbruikbaar zijn en volledig op rate limiting/lockout leunen (zie ook Taak 6 stap 4). Let op: het bestandspad heet `model/unique~code.js` (met tilde), niet `unique-code.js`.
>
> 💬 **REVIEW-OPMERKING, MEDIUM (testrunner).** Deze taak schrijft "unit tests" met de aanname vitest, en §8 valideert met `npm run test:unit:auth`. Maar `apps/auth-server` draait **jest** (`jest unit`; bestaande tests o.a. `apps/auth-server/utils/sanitize.test.js`). Het root-script `test:unit:auth` = `vitest run apps/auth-server`, wat over de jest-testfiles struikelt. Schrijf nieuwe auth-server tests als jest en valideer met de app-lokale `npm test`. Zelfde punt geldt voor Taak 5 (`apps/api-server` draait ook jest, `jest --config jest.config.js`).
...
> 💬 **REVIEW-OPMERKING, CRITICAL (stap 3).** `checkClientGates` retourneert hieronder alleen `uniquecode_required` en `fields_required`. Voeg `phonenumber_required` toe en behandel de 2FA-uitzondering, zoals uitgewerkt in de review-opmerking bij "Beleidsregel voor stille exchange" in sectie 4. Zonder die twee gates is de exchange een privilege-escalatie over projecten heen.
>
> 💬 **REVIEW-OPMERKING, LOW (footgun tijdens dit werk).** `check2FA` leest `req.currentClientAuth?.twoFactorValid || req.session?.twoFactorValid` (`apps/auth-server/middleware/client.js:229`). De tweede tak is een globale sessie-vlag die vandaag NERGENS wordt gezet (2FA-validiteit staat alleen per client in `session.clientAuth[clientId]`), dus inert. Maar zodra iemand tijdens dit multi-project werk per ongeluk een top-level `session.twoFactorValid` zet, wordt het een directe globale 2FA-bypass over alle projecten. Verwijder die fallback als je hier toch in de buurt zit.
...
> 💬 **REVIEW-OPMERKING, MEDIUM.** De aanname dat rate limiting stemcode-brute-force afdekt is optimistisch. Twee feiten: (1) brute-force staat volledig UIT: `apps/api-server/src/middleware/brute-force.js` bevat alleen lege `next()`-stubs (express-brute jaren geleden uitgecommentarieerd). (2) `packages/lib/rateLimiter.js` gebruikt `express-rate-limit` met de default in-memory store en IP-key, per route-instantie: dus niet per project, niet gedeeld tussen api-instances (limiet = `max x instances`), en private IPs worden overgeslagen. Achter NAT deelt een heel kantoor één budget. Combineer met de herbruikbare codes (zie Taak 4 stap 2) en dit is een reëel brute-force-oppervlak. Voeg een per-code/per-project teller of lockout in de DB toe, niet alleen de IP-limiter.
...
- [ ] **Stap 2: state-update helper in `use-current-user.js`**
  ...
  > 💬 **REVIEW-OPMERKING, MEDIUM.** `use-current-user.js` destructureert geen `mutate` uit `useSWR`, dus `await mutate(...)` werkt niet zoals geschreven. Het vaste revalidatie-pad is `self.refresh()` (`packages/data-store/src/index.js:142-154`), aan te roepen na het zetten van `self.api.currentUserJWT = jwt`. Gebruik dat i.p.v. een lokale `mutate`.
...
> 💬 **REVIEW-OPMERKING, MEDIUM.** In `likes.tsx` zet `doVote` `isBusy=true` (`:118-119`) vóór de rol-check, en de huidige redirect-branch reset dat nooit (mocht, want de pagina navigeerde weg). Als je de redirect vervangt door een awaited `requireLogin()` die bij annuleren `null` resolvet, blijft de widget in busy-state hangen. Wikkel het nieuwe pad in try/finally met `setIsBusy(false)`. Check hetzelfde patroon in stem-begroot.
...
> 💬 **REVIEW-OPMERKING, MEDIUM.** Dat pad bestaat niet. De Cypress e2e-setup is root-level: `cypress.config.ts` + `cypress/e2e/1-smoke-test/admin.cy.js`, gedraaid met `npm run test:e2e` (`cypress run`). Widget-tests zijn Cypress component-tests onder `packages/*/cypress/component/`. Plaats de nieuwe spec in root `cypress/e2e/`, niet onder `apps/admin-server/`.
...
## 7bis. Voorgestelde extra taken (REVIEW-OPMERKINGEN, nog niet in het plan)

> 💬 Deze hele sectie is commentaar van de reviewer. Dit zijn multi-project-zaken die het plan nu niet adresseert. De eerste drie zijn ontwerpvragen die pas bestaan zodra je in meerdere projecten tegelijk kunt inloggen; die horen bij het bouwen van de feature. De laatste twee zijn correctheids-/opruimtaken.

- [ ] **Extra taak A, HIGH: identiteits-seeding fixen (`globalOpenStadUser`).** Vandaag seedt elke widget zijn current user uit één page-brede `window.globalOpenStadUser` (`packages/data-store/src/hooks/use-current-user.js:48,95-97`), gezet vanuit één projectsessie (CMS: `apps/cms-server/modules/openstad-auth/index.js:102`; admin-preview: `apps/admin-server/src/components/widget-preview.tsx:25`). Op een multi-project pagina pakt widget B daardoor de JWT van project A en stuurt die als `Authorization: Bearer` naar B's API. De hook moet de identiteit uit de eigen project-namespace (`openstad[projectId].openStadUser`) halen en de global alleen gebruiken als die bij het eigen `projectId` hoort. Dit is een voorwaarde voordat de auth-broker (Taak 7) veilig is: de localStorage-kant is al netjes per project genamespaced, deze JS-global is het lek.

- [ ] **Extra taak B, HIGH: logout-semantiek voor multi-project beslissen.** Nu doet `apps/auth-server/controllers/auth/local.js:186-200` `req.session.destroy()` bij logout van één client, wat de gedeelde SSO-sessie sloopt (alle `clientAuth[*]`). De widget-logout redirect naar exact deze flow (`apps/api-server/src/adapter/openstad/router.js:451-481`). Gevolg: uitloggen bij project A logt je ook uit B en C. Kies en implementeer: per-project logout (`clearClientAuth(session, client)` + tokens van die client, en pas `session.destroy()` als er geen clients meer over zijn) OF "globale logout" als bewuste, gedocumenteerde keuze.

- [ ] **Extra taak C, MEDIUM-HIGH: session-regeneration bij login.** De auth-server roept nergens `req.session.regenerate()` aan, en `saveUninitialized: true` (`apps/auth-server/app-init.js:112`) mint al een sessie voor anonieme bezoekers. Bij een gedeelde, lang-levende SSO-sessie (tot 7 dagen, `apps/auth-server/utils/clientAuth.js:7-8`) over meerdere projecten wordt session fixation extra impactvol: één gefixeerde `openstad-authorization.sid` geeft toegang tot elk project waar het slachtoffer daarna op inlogt. Regenereer de sessie-id rond het inloggen.
